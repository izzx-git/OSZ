...
ESP_PACKET_MAX equ 3000 ;предполагаемый максимальный размер пакета от ESP. Размер может быть разным на разных прошивках.
...
;вх: hl - адрес для данных (рекомендуется зарезервировать место не меньше размера ESP_PACKET_MAX)
;вых: CY=0 - OK; CY=1 - занято другим процессом или нет uart
...
;экран 5 использовать не желательно, там может быть буфер сети ESP или приложение в монопольном режиме (см. OS_SET_MONO_MODE)
    macro OS_SET_SCREEN ;
    ld c,#1d
    rst #20
    endm	
