...
init:
    ;ld hl, .uartIniting : OS_PRINTZ
    ;call Uart.init ;инициализация уже сделана системой
    ld hl, .chipIniting : OS_PRINTZ
    EspCmdOkErr "ATE0"
    jr c, .initError
    ...
    ld hl, .doneInit : OS_PRINTZ
    
    or a
    ret
.initError
    ld hl, .errMsg : OS_PRINTZ
    scf
    ret
...
.readp
    ld a, h : cp #ff : jr nc, .skipbuff ;ограничение буфера #ff00
    push bc, hl
    call Uart.read
    pop hl, bc
    ld (hl), a
